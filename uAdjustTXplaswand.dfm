object MainForm: TMainForm
  Left = 266
  Top = 236
  Caption = 'MainForm'
  ClientHeight = 633
  ClientWidth = 854
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 160
    Top = 32
    Width = 48
    Height = 13
    Caption = 'Flx bottom'
  end
  object Label2: TLabel
    Left = 160
    Top = 56
    Width = 46
    Height = 13
    Caption = 'PlasRand'
  end
  object Label3: TLabel
    Left = 177
    Top = 88
    Width = 29
    Height = 13
    Caption = 'TXorg'
  end
  object Label4: TLabel
    Left = 168
    Top = 128
    Width = 37
    Height = 13
    Caption = 'f (d2/m)'
  end
  object Laagdikte: TLabel
    Left = 144
    Top = 168
    Width = 64
    Height = 13
    Caption = 'Laagdikte (m)'
  end
  object GoButton: TButton
    Left = 768
    Top = 592
    Width = 75
    Height = 25
    Caption = 'Go'
    TabOrder = 0
    OnClick = GoButtonClick
  end
  object Memo1: TMemo
    Left = 176
    Top = 280
    Width = 473
    Height = 273
    Lines.Strings = (
      
        'MIPWA-tool voor modelleren zandwinplas waarbij door infiltratie ' +
        'een extra weerstand ontstaat '
      'op de plaswand. Zie ook: AdjustCbodem.'
      
        'De extra weerstand is evenredig verondersteld aan de sterkte van' +
        ' de infiltratieflux (m/d).'
      ''
      
        'Het doorlaatvermogen van de cellen in de plaswand wordt navenant' +
        ' verkleind. '
      ''
      
        'Alle bestanden in idf-formaat. NIET NOODZAKELIJKERWIJS IN DEZELF' +
        'DE RESOLUTIE.'
      
        'Uitvoerbestand heeft dezelfde resolutie en afmetingen als het id' +
        'f-bestand met de '
      'oorspronkelijke weerstanden.'
      ''
      'Invoerbestanden:'
      
        'ParamStr( 1 ): BDGFLx (idf-bestand als berekeningsresultaat van ' +
        'MIPWA (m3/d))'
      'ParamStr( 2 ): Plasrand lokatie (als waarde > 0); '
      'ParamStr( 3 ): Oorspronkelijk doorlaatvermogen (m2/d)'
      'ParamStr( 4 ): Evenredigheidsconstante (d2/m)'
      'ParamStr( 5 ): Laagdikte (m) '
      
        'ParamStr( 6 ): Idf-(uitvoer) bestand met verlaagde doorlaatvermo' +
        'gens op de plasrand (m2/d)'
      ''
      'KVI 1-2-2010')
    TabOrder = 1
  end
  object EditflxBottom: TEdit
    Left = 216
    Top = 24
    Width = 625
    Height = 21
    TabOrder = 2
    Text = 'EditflxBottom'
    OnClick = EditflxBottomClick
  end
  object EditplasRand: TEdit
    Left = 216
    Top = 56
    Width = 625
    Height = 21
    TabOrder = 3
    Text = 'EditplasRand'
    OnClick = EditplasRandClick
  end
  object EditTXorg: TEdit
    Left = 216
    Top = 88
    Width = 625
    Height = 21
    TabOrder = 4
    Text = 'EditTXorg'
    OnClick = EditTXorgClick
  end
  object ESBPosFloatEdit_F: TESBPosFloatEdit
    Left = 216
    Top = 128
    Width = 81
    Height = 21
    TabOrder = 5
    OnChange = ESBPosFloatEdit_FChange
    Scale = 1.000000000000000000
    AsFloat = 2250.000000000000000000
    DecimalPlaces = 0
  end
  object ESBPosFloatEditLaagdikte: TESBPosFloatEdit
    Left = 216
    Top = 160
    Width = 81
    Height = 21
    TabOrder = 6
    OnChange = ESBPosFloatEditLaagdikteChange
    Scale = 1.000000000000000000
    AsFloat = 10.000000000000000000
    DecimalPlaces = 1
  end
  object SingleESRIgridFLxBottom: TSingleESRIgrid
    Left = 784
    Top = 16
  end
  object SingleESRIgridPlasRand: TSingleESRIgrid
    Left = 784
    Top = 56
  end
  object SingleESRIgridTXorg: TSingleESRIgrid
    Left = 784
    Top = 80
  end
  object OpenDialogflxBottom: TOpenDialog
    DefaultExt = 'idf'
    FileName = 'bdgflfBottom.idf'
    Filter = '*.idf|*.idf'
    Title = 'Open BDGFL bottom (idf-file)'
    Left = 424
    Top = 24
  end
  object OpenDialogPlasRand: TOpenDialog
    DefaultExt = 'idf'
    FileName = 'Plasrand.idf'
    Filter = '*.idf|*.idf'
    Title = 'Open plasrand.idf'
    Left = 424
    Top = 56
  end
  object OpenDialogTXorg: TOpenDialog
    DefaultExt = 'idf'
    FileName = 'TXorg.idf'
    Title = 'Open TX-org'
    Left = 424
    Top = 88
  end
  object SaveDialog1: TSaveDialog
    DefaultExt = 'idf'
    FileName = 'cResult.idf'
    Filter = '*.idf|*.idf'
    Options = [ofOverwritePrompt, ofHideReadOnly, ofEnableSizing]
    Title = 'Specify cResult.idf'
    Left = 720
    Top = 592
  end
end
